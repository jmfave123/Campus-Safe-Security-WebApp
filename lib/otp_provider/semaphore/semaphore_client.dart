...
import 'semaphore_models.dart';

/// Semaphore SMS client for sending OTP messages.
///
/// This implementation uses Semaphore's OTP-specific endpoint which
/// provides better delivery rates for OTP traffic compared to regular SMS.
class SemaphoreClient implements OtpProvider {
  ...
  /// Send OTP request to Vercel backend (no CORS!)
  Future<http.Response> _sendOtpRequest({
    ...
    _debugLog('sendOtp (via Vercel)', response);
    return response;
  }

  /// Handle the response from Semaphore send request
  SendResult _handleSendResponse(http.Response response) {
    final decoded = _tryDecode(response.body);

    if (response.statusCode == 200 || response.statusCode == 201) {
      try {
        // Handle Vercel API response format: {success: true, message: "...", data: {...}}
        if (decoded is Map<String, dynamic> && decoded['success'] == true) {
          final data = decoded['data'];

          // Extract OTP code from Semaphore response (nested in data)
          String? otpCode;
          if (data is Map<String, dynamic>) {
            // Try to get OTP code from various possible fields
          ...
          return SendResult(
            success: true,
            providerMessageId:
                'vercel_${DateTime.now().millisecondsSinceEpoch}',
            code: otpCode, // Real OTP code from Semaphore via Vercel
            message: decoded['message']?.toString() ?? 'OTP sent successfully',
            rawResponse: _redactSensitiveData(decoded),
          );
        }

        // Fallback: try parsing as direct Semaphore response (for backward compatibility)
        final semaphoreResponse = SemaphoreSendResponse.fromJson(decoded);

        return SendResult(
          success: semaphoreResponse.isSuccess,
          providerMessageId:
              semaphoreResponse.firstMessage?.messageId.toString(),
          code: semaphoreResponse.otpCode,
          message: semaphoreResponse.statusMessage,
          rawResponse: _redactSensitiveData(decoded),
        );
      } catch (e) {
        throw ProviderException(
          'Failed to parse Semaphore response: $e',
